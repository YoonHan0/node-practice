...
3. 패키지 설치
    ...
    3) 패키지 설치 실습
        ...
        $ npm i -D nodemon  [local, dev, dependancy]            // -D : 빌드할 때 포함할 것이냐
        $ npm i -g gulp     [global, general, dependency]       // global은 package.json에 명시되지 않음
        $ npx gulp --version
    4) 패키지 삭제
        $ npm un ejs        [local install 삭제]
        $ npm un nodemon    [local install 삭제]
        $ npm un -g gulp    [global install 삭제]
        $ npx gulp --version    // 삭제됐는지 확인(실행)

4. Node(JavaScript) Project 생성
    1) 프로젝트 생성(mkdir)
    2) 프로젝트 이동(cd)
    3) 프로젝트 초기화: 프로젝트 메니페스트(manifest) 파일인 package.json을 생성
        $[project-ex01] npm init -y
5. Module
    1) 코어 모듈: node에서 제공하는 모듈(fs, os, process, http, ...)
    2) 파일 모듈: 파일 경로로 불러온 모듈안의 함수, 객체를 사용한다.
    3) npm 모듈: np을 통해서 node_modules에 설치하고 사용하는 모듈
        - 원격 배포가 된 패키지
            npmjs.org의 npm registry에 패키지 배포
...
6. helloweb-ex01: 웹어플리케이션 만들어 보기(express 프레임워크 x)
7. helloweb-ex02: 웹어플리케이션 만들어 보기(express 프레임워크 o)